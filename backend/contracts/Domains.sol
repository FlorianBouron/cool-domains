...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Lambo Moon Name Service", "LMNS")
    {
        tld = _tld;
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 1 * 10**17; // 1 MATIC = 1 000 000 000 000 000 000 (18 decimals). We're going with 0.1 Matic cause the faucets don't give a lot
        } else if (len == 4) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Lambo Moon name service", "image": "data:image/svg+xml;base64,',
